...
$(document).ready(function () {
  ...
  $('#reply-title').addClass('h4'); // Featured image

  $('.post-thumbnail>img').addClass('img-thumbnail img-fluid shadow mx-auto d-block ');
  $('.post-thumbnail>img').css('max-height', '200px');
  $('.post-thumbnail>img').css('width', 'auto'); // Custom logo link image

  $('.custom-logo-link>img').css('max-height', '50px');
  $('.custom-logo-link>img').css('width', 'auto'); // Footer widgets

  $('.site-footer a').addClass('unlink');
  $('footer ul, footer li').css('list-style-type', 'none');
  $('footer ul').css('margin', '0px'); // Posts navigation links
